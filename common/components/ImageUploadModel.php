...
class ImageUploadModel extends FileUploadModel
{
    ...
    public function rules()
    {
        return array(
            ...
            array('file', 'file', 'extensions' => ['jpg', 'png', 'gif', 'bmp', 'jpe', 'jpeg', 'jpeg'])
        );
    }
    ...
    protected function processThumbnailsImage($param)
    {
        ...
        if (!empty($param['is_tumb'])) {
            $this->tumbFile = $param['path'];
        }
    ...
    public function getTumbUrl()
    {
        return  Storage::getStorageUrlTo($this->getTumbFile()) . DS . $this->getFileName();
    }
    ...
    public function getTumbFile()
    {
        return (!empty($this->tumbFile)) ? $this->tumbFile : $this->uploadParams['origin_path'];
    }
}