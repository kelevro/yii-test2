...
(function($)
{
	...
	Redactor.fn = $.Redactor.prototype = {
		...
		imageEdit: function(e)
		{
			...
			var callback = $.proxy(function()
			{
                $('#redactor_file_height').val($el.css('height'));

                $('#redactor_file_width').val($el.css('width'));
		...
		imageSave: function(el)
		{
            ...
            if ($('#redactor_file_width').val()) {
                $(el).css('width', $('#redactor_file_width').val().replace('px', '') + 'px');
            }
            if ($('#redactor_file_height').val()) {
                $(el).css('height', $('#redactor_file_height').val().replace('px', '') + 'px');
            }
			this.modalClose();
			this.observeImages();
			this.sync();
		...
		modalTemplatesInit: function()
		{
			$.extend( this.opts,
			{
				...
				+ '<section>'
					...
					+ '<input id="redactor_file_height" class="redactor_input" />'
